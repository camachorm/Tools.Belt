...
{
    public class UserCredentialsTokenService : TokenServiceBase, IUserCredentialsTokenService
    {
        public UserCredentialsTokenService(
            string clientId, 
            IEnumerable<string> scopes,
            AzureCloudInstance azureCloudInstance, 
            AadAuthorityAudience aadAuthorityAudience,
            ILogger<UserCredentialsTokenService> logger,
            string tenantId) 
            : base(clientId, 
                scopes,
                azureCloudInstance, 
        ...
        public override async Task LoginAsync()
        {
            ...
            {
                AuthenticationResult = await GetTokenForWebApiUsingInteractiveModeAsync(Scopes);

                if (!IsAuthenticated)
                {
                    Logger.LogError("Unable to authenticate, please try again or hit CTRL+C to abort.");
                }
            } 
        }

        protected override void InitializeClientApplication()
        {
            ...
            ApplicationBase = PublicClientApplicationBuilder.CreateWithApplicationOptions(options).WithRedirectUri("http://localhost").Build();
        }

        // ReSharper disable MemberCanBePrivate.Global
        public IPublicClientApplication ClientApplication => ApplicationBase as IPublicClientApplication;
        ...
        private async Task<AuthenticationResult> GetTokenForWebApiUsingInteractiveModeAsync(IEnumerable<string> scopes)
        {
